...
class ExposedLSTMLayer(MergeLayer):
    """
    Lasagne's LSTMLayer, but with the memory exposed
    
    To use this layer:
    >>> l_lstm = ExposedLSTMLayer(input_layer,
        num_units,
    ...
    >>> cell_out = SliceLayer(l_lstm, indices=slice(0, num_units), axis=-1)
    >>> hid_out = SliceLayer(l_lstm, indices=slice(num_units, None), axis=-1)
    """
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        return T.concatenate([cell_out, hid_out], axis=-1)
